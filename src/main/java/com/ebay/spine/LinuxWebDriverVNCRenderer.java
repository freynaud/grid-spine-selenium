...
public class LinuxWebDriverVNCRenderer implements HtmlRenderer {
	private LinuxWebDriverVNCProxy proxy;
	private Registry registry;
	public static boolean debug = false;

	public LinuxWebDriverVNCRenderer() {
		throw new RuntimeException("No.");
	}

	public LinuxWebDriverVNCRenderer(LinuxWebDriverVNCProxy proxy) {
		this.proxy = proxy;
	...
	public String renderSummary() {
		...
		.append("<legend>")
		.append("<img width='30' src='../resources/images/tux.png' style='vertical-align:middle;' title='"+proxy.getVm().getName()+"'/>")
		...
		for (TestSlot slot : proxy.getTestSlots()) {
			...
			if (session != null) {		
				builder.append(" class='vncCapable' hub='"+proxy.getRegistry().getHub().getHost()+"' pwd='"+proxy.getVncPassword()+"' ip='"+proxy.getRemoteURL().getHost()+"' display='"+session.getRequestedCapabilities().get("x")+"' ");
